...
with TinyDB(file, indent=4, separators=(', ', ': ')) as db:
    ...
    db.insert({'session_name': MAIN, 'window_name': tutorial_interactive_programming_languages, 'same_pane': True,
               'send_enter': 1, "pre_arg": "php -a", 'exec_script': 'PHP_code_with_errors', 'args': ['\nexit']})
    db.insert({'session_name': MAIN, 'window_name': tutorial_interactive_programming_languages, 'same_pane': False,
               'send_enter': 1, 'exec_script': 'Python_code_with_errors', "pre_arg": "ipython3", 'args': ['string_example)\nexit()']})
    db.insert({'session_name': MAIN, 'window_name': tutorial_interactive_programming_languages, 'same_pane': True,
               'send_enter': 1, 'exec_script': 'POWERSHELL_code_with_errors', "pre_arg": "pwsh", 'args': ['\nexit']})
